...
#include "cpu/aarch64/kdnn/kdnn_inner_product.hpp"
#endif
...
const std::map<pk_dt_impl_key_t, std::vector<impl_list_item_t>> &impl_list_map() {
    static const std::map<pk_dt_impl_key_t, std::vector<impl_list_item_t>> the_map = REG_IP_P({
        {{forward, f32, f32, f32}, {
            CPU_INSTANCE_AARCH64_KDNN(kdnn_inner_product_fwd_t)
        ...
        {{backward_data, f32, f32, f32}, REG_BWD_PK({
            CPU_INSTANCE_AARCH64_KDNN(kdnn_inner_product_bwd_data_t)
        ...
        {{backward_weights, f32, f32, f32}, REG_BWD_PK({
            CPU_INSTANCE_AARCH64_KDNN(kdnn_inner_product_bwd_weights_t)
