#ifndef KDNN_JIT_GENERATOR_HPP
#define KDNN_JIT_GENERATOR_HPP
...
#define DECLARE_KDNN_JIT_AUX_FUNCTIONS(jit_name) \
...
constexpr Xbyak_aarch64::Operand::Code kdnn_abi_save_gpr_regs[]
...
static const Xbyak_aarch64::XReg kdnn_abi_param1(Xbyak_aarch64::Operand::X0),
        kdnn_abi_param2(Xbyak_aarch64::Operand::X1),
        kdnn_abi_param3(Xbyak_aarch64::Operand::X2),
        kdnn_abi_param4(Xbyak_aarch64::Operand::X3),
        kdnn_abi_param5(Xbyak_aarch64::Operand::X4),
        kdnn_abi_param6(Xbyak_aarch64::Operand::X5),
        kdnn_abi_param7(Xbyak_aarch64::Operand::X6),
        kdnn_abi_param8(Xbyak_aarch64::Operand::X7),
        kdnn_abi_not_param1(Xbyak_aarch64::Operand::X15);
} // namespace

class kdnn_jit_generator : public Xbyak_aarch64::CodeGenerator, public c_compatible {
public:
...
private:
    ...
    const size_t num_abi_save_gpr_regs
            = sizeof(kdnn_abi_save_gpr_regs) / sizeof(kdnn_abi_save_gpr_regs[0]);
...
public:
    ...
    const Xbyak_aarch64::XReg param1 = kdnn_abi_param1;
    ...
    void preamble() {
        ...
        for (size_t i = 0; i < num_abi_save_gpr_regs; i += 2) {
            stp(Xbyak_aarch64::XReg(kdnn_abi_save_gpr_regs[i]),
                    Xbyak_aarch64::XReg(kdnn_abi_save_gpr_regs[i + 1]),
    ...
    void postamble() {
        ...
        for (size_t i = 0; i < num_abi_save_gpr_regs; i += 2) {
            ldp(Xbyak_aarch64::XReg(kdnn_abi_save_gpr_regs[i]),
                    Xbyak_aarch64::XReg(kdnn_abi_save_gpr_regs[i + 1]),
                    post_ptr(x9, xreg_len * 2));
        }
    ...
    DNNL_DISALLOW_COPY_AND_ASSIGN(kdnn_jit_generator);

public:
    kdnn_jit_generator(void *code_ptr = nullptr, size_t code_size = 256 * 1024,
            bool use_autogrow = true)
        : Xbyak_aarch64::CodeGenerator(code_size,
                (code_ptr == nullptr && use_autogrow) ? Xbyak_aarch64::AutoGrow
                                                      : code_ptr) {}
    virtual ~kdnn_jit_generator() {}
...
private:
    const uint8_t *getCode() {
        ...
        register_jit_code(code, getSize() * sizeof(uint32_t));
        return code;
    }
