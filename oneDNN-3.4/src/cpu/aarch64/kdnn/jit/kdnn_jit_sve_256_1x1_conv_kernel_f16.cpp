...
status_t kdnn_jit_sve_256_1x1_conv_kernel_f16::init_conf(kdnn_jit_1x1_conv_conf_t &jcp,
            const convolution_desc_t &cd, memory_desc_t &src_pd,
            memory_desc_t &weights_pd, memory_desc_t &dst_pd, 
            const primitive_attr_t &attr, int nthreads, bool reduce_src) {
    /* arch check */
    if (!mayiuse(sve_256)) return status::unimplemented;
    ...
    if (!everyone_is(data_type::f16, src_d.data_type(), weights_d.data_type(),
                dst_d.data_type()))
    ...
    const auto dat_tag_nCx16c = utils::pick(ndims - 3, nCw16c, nChw16c, nCdhw16c);
    const auto dat_tag_ncx = utils::pick(ndims -3, ncw, nchw, ncdhw);
    jcp.src_tag = src_d.matches_one_of_tag(dat_tag_nxc, dat_tag_nCx16c, dat_tag_ncx);
    jcp.dst_tag = dst_d.matches_one_of_tag(dat_tag_nxc, dat_tag_nCx16c, dat_tag_ncx);
    bool is_data_layout_nxc
            = utils::everyone_is(dat_tag_nxc, jcp.src_tag, jcp.dst_tag);
    bool is_src_data_layout_ncx
            = utils::everyone_is(dat_tag_ncx, jcp.src_tag);
    bool is_dst_data_layout_ncx
            = utils::everyone_is(dat_tag_ncx, jcp.dst_tag);
    bool is_dst_data_layout_nxc
            = utils::everyone_is(dat_tag_nxc, jcp.dst_tag);
    
    /*if data layout is ncx, need to change it to nxc*/
    if(!is_data_layout_nxc && is_src_data_layout_ncx) {
        is_dst_data_layout_ncx == true ? jcp.dst_tag = jcp.src_tag = dat_tag_nxc :
                                         jcp.src_tag = dat_tag_nxc;
        is_data_layout_nxc = true;
    }
    auto required_dat_tag = is_data_layout_nxc ? dat_tag_nxc : dat_tag_nCx16c;
    //when dst tag is nxc or ncx, required dat tags are nxc, for ncx need be changed to nxc
    auto required_dst_dat_tag = (is_dst_data_layout_ncx || is_dst_data_layout_nxc) ? dat_tag_nxc : dat_tag_nCx16c;

    /* Channel padding check */
    bool ok_to_pad_channels = true && !is_data_layout_nxc && jcp.ngroups == 1
            && src_d.data_type() == data_type::f16;
    ...
    bool args_ok = true && jcp.ngroups == 1 && jcp.src_tag == required_dat_tag
            && jcp.dst_tag == required_dst_dat_tag
    ...
    jcp.ver = kdnn_ver_sve_256;
    if (everyone_is(data_type::f16, src_d.data_type(), weights_d.data_type(),
                dst_d.data_type())) {
        const int is_bwd_d = jcp.prop_kind == backward_data;
        /* Set weight data layout tag */
        format_tag_t wei_tag = with_groups
            ? utils::pick(2 * ndims - 6 + is_bwd_d, gOIw16i16o, gOIw16o16i,
                    gOIhw16i16o, gOIdhw16o16i, gOIhw16i16o, gOIdhw16o16i)
            : utils::pick(2 * ndims - 6 + is_bwd_d, OIw16i16o, OIw16o16i, OIhw16i16o,
                    OIhw16o16i, OIdhw16i16o, OIdhw16o16i);

        jcp.wei_tag = weights_d.matches_one_of_tag(wei_tag);
        if (jcp.wei_tag != wei_tag) return status::unimplemented;

        //        jcp.fma_step = 1;
        jcp.typesize_in = sizeof(prec_traits<data_type::f16>::type);
        jcp.typesize_out = sizeof(prec_traits<data_type::f16>::type);
    } else {
    ...
    const int BIG_LOAD_DIM = (jcp.reduce_dim >= 256) ? 128 : 256;
    ...
    const int L1_capacity
            = platform::get_per_core_cache_size(1) / sizeof(float16_t);
    ...
    if (one_of(jcp.prop_kind, forward_training, forward_inference,
        ...
        //jcp.ur modify for regs mostest in use
        if(jcp.oc <= 32)
        {
            jcp.ur = 10;
        }
        else if(jcp.oc >= 64 && jcp.oc <= 128)
        {
            jcp.ur = 6;
        }
        else if(jcp.oc >= 1024 || 
               (jcp.oc < 320 && jcp.oc >= 256) ||
               (jcp.oc >= 512 && jcp.ic <= 128))
        {
            jcp.ur = 4;
        }
        else if((jcp.oc >= 512 && jcp.ic > 128) ||
                (jcp.oc >= 320 && jcp.oc < 512))
        {
            jcp.ur = 5;
        }
        ...
        if (jcp.prop_kind == backward_data)
            jcp.loop_order = kdnn_loop_lbr;
        else
            jcp.loop_order = reduce_src ? kdnn_loop_blr : kdnn_loop_lbr;
        ...
        if (is_data_layout_nxc) {
            ...
            if (spatial <= SMALL_SPATIAL && jcp.reduce_dim >= BIG_REDUCE_DIM)
                ...
                reduce_blocking = 8;
            reduce_blocking = kdnn_best_divider(nb_reduce, 1, reduce_blocking, true);
        ...
        if (reduce_blocking < jcp.reduce_dim) {
            if (jcp.prop_kind == backward_data)
                jcp.loop_order = reduce_src ? kdnn_loop_lbr : kdnn_loop_rlb;
            else
                jcp.loop_order = reduce_src ? kdnn_loop_rbl : kdnn_loop_rlb;
        }
        ...
        if (jcp.nthr <= 12 && jcp.mb < jcp.nthr
            ...
            // Some heuristic here
            float16_t calc_koef = 0.01, best_cost = FLT_MAX;
            int n_lgc = jcp.nthr;
            float16_t ratio = (float16_t)load_size / (float16_t)bcast_size;
            int best_lgc = ratio > 1 ? n_lgc : 1;
            auto calc_job_cost = [&](int lb, int tg, float16_t mem_k) {
                int bb_size = jcp.mb * div_up(nb_bcast, tg);
                float16_t calc_size = (float16_t)(bb_size * jcp.ur)
                        * (lb * jcp.load_block) * jcp.reduce_dim;
                float16_t mem_size = (float16_t)(bb_size * jcp.ur + lb * jcp.load_block)
                        * jcp.reduce_dim;
            ...
            for (int lgc, ilgc = 0; ilgc < n_lgc; ilgc++) {
                ...
                // Some heuristic here
                float16_t mem_koef = (max_tg == 1) ? 1.f : 1.3f;
                float16_t job_cost = 0.;
            ...
            jcp.load_grp_count = kdnn_best_divider(jcp.nthr, jcp.load_grp_count,
        ...
        auto get_thr_eff = [=](int load_chunk, int nthr) {
            ...
            float16_t data_norm = (bcast_per_thr + load_per_thr) / 2.f;
            float16_t data_eff
                    = (bcast_per_thr * load_per_thr) / (data_norm * data_norm);
            float16_t thr_eff_over_grp
                    = (float16_t)nstl::max(1, nthr / lgc) / div_up(nthr, lgc);
            float16_t thr_eff_in_grp = ((float16_t)jcp.mb * nb_bcast)
                    / rnd_up(jcp.mb * nb_bcast, thr_per_grp);
            float16_t thr_eff = thr_eff_over_grp * thr_eff_in_grp;
            float16_t load_eff = (float16_t)nb_load / rnd_up(nb_load, lgc);
            float16_t overall_eff = data_eff + thr_eff + load_eff;
            return overall_eff;
        };

        auto get_load_chunk = [=](int nthr) {
            float16_t best_eff = -1.0f;
            int best_lgc = 1;
            float16_t eff;
        ...
        if (sizeof(float16_t) * bcast_size < 8192 && jcp.mb < jcp.nthr
                && nb_load * nb_bcast < jcp.nthr) {
            float16_t best_thr_eff = -1.0f;
            float16_t thr_eff = -1.0f;
        ...
        jcp.reduce_block = kdnn_best_divider(jcp.reduce_dim, 7, 16, true);
        if (jcp.reduce_dim % jcp.reduce_block != 0)
            jcp.reduce_block = kdnn_best_divider(jcp.iw, 4, jcp.iw, false);
        ...
        load_blocking = kdnn_best_divider(load_blocking, 16, load_blocking, false);
        load_blocking *= jcp.load_block;
        ...
        bcast_blocking = kdnn_best_divider(
        ...
        if (is_data_layout_nxc && jcp.reduce_dim >= BIG_SPATIAL * BIG_SPATIAL
            ...
            reduce_blocking = kdnn_best_divider(jcp.reduce_dim, min_reduce_blocking,
...
void kdnn_jit_sve_256_1x1_conv_kernel_f16::init_scratchpad(
...
/* BWD W*/
void kdnn_jit_sve_256_1x1_conv_kernel_f16::balance(kdnn_jit_1x1_conv_conf_t &jcp) {
