#ifndef KDNN_JIT_F16_SVE_CONV_KERNEL_HPP
#define KDNN_JIT_F16_SVE_CONV_KERNEL_HPP
...
#define KDNN_F16_VL_OFS(ofs) (ofs >> 4)
...
struct kdnn_jit_f16_sve_conv_fwd_kernel : public kdnn_jit_generator {

    kdnn_jit_f16_sve_conv_fwd_kernel(
            const kdnn_jit_conv_conf_t &ajcp, const primitive_attr_t &attr)
        : jcp(ajcp), attr_(attr) {}

    DECLARE_KDNN_JIT_AUX_FUNCTIONS(kdnn_jit_f16_sve_conv_fwd_kernel)
...
private:
    ...
    void prefetch(
        ...
        if (cacheline_aligned == true) {
            ...
            if ((KDNN_F16_VL_OFS(ofs) < PRFWMAX)
                    && (KDNN_F16_VL_OFS(ofs) >= (-1 * PRFWMAX))) {
                prfw(op_sve, P_ALL_ONE,
                        ptr(in, static_cast<int32_t>(KDNN_F16_VL_OFS(ofs))));
            } else {
                add_imm(reg_tmp_addr, in, ofs, reg_tmp_imm);
