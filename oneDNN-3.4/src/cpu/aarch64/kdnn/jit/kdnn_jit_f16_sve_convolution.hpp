#ifndef KDNN_JIT_F16_SVE_CONVOLUTION_HPP
#define KDNN_JIT_F16_SVE_CONVOLUTION_HPP
...
struct kdnn_jit_f16_sve_convolution_fwd_t : public primitive_t {
    struct pd_t : public cpu_convolution_fwd_pd_t {
        ...
        DECLARE_COMMON_PD_T("kdnn_f16_jit", kdnn_jit_f16_sve_convolution_fwd_t);
        ...
        status_t init(engine_t *engine) {
            ...
            status_t status = kdnn_jit_f16_sve_conv_fwd_kernel<isa>::init_conf(jcp_,
                    *desc(), src_md_, weights_md_, dst_md_, bias_md_, *attr(),
                    dnnl_get_max_threads());
            if (status != status::success) return status;

            auto scratchpad = scratchpad_registry().registrar();
            kdnn_jit_f16_sve_conv_fwd_kernel<isa>::init_scratchpad(scratchpad, jcp_);
    ...
    kdnn_jit_f16_sve_convolution_fwd_t(const pd_t *kpd) : primitive_t(kpd) {}

    status_t init(engine_t *engine) override {
        CHECK(safe_ptr_assign(kernel_,
                new kdnn_jit_f16_sve_conv_fwd_kernel<isa>(pd()->jcp_, *pd()->attr())));
...
private:
    ...
    void prepare_padded_bias(const float16_t *&bias,
    ...
    std::unique_ptr<kdnn_jit_f16_sve_conv_fwd_kernel<isa>> kernel_;

}; // kdnn_jit_f16_sve_convolution_fwd_t
