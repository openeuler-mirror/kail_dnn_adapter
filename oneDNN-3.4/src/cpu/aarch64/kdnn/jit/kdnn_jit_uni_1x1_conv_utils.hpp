...
#ifndef CPU_AARCH64_KDNN_JIT_UNI_1X1_CONV_UTILS_HPP
#define CPU_AARCH64_KDNN_JIT_UNI_1X1_CONV_UTILS_HPP
...
struct kdnn_reduce_to_unit_stride_t {
...
inline void kdnn_rtus_prepare(conv_pd_t *self, const convolution_desc_t *&conv_d,
    ...
    if (ndims == 3)
        rtus_applicable = rtus_applicable && conv_d->strides[0] != 1
                && conv_d->src_desc.data_type != data_type::f16;
    else
    ...
    const auto dat_tag = ndims == 3
            ? memory_desc_wrapper(src_d).matches_one_of_tag(
                    format_tag::nCw8c, format_tag::nCw16c, format_tag::nwc) // add nxc for kernel has surported dealing nxc
            : memory_desc_wrapper(src_d).matches_one_of_tag(
...
template <typename conv_pd_t>
inline void kdnn_rtus_prepare_space_info(conv_pd_t *self,
        memory_tracking::registrar_t &scratchpad, int max_threads) {
    if (!self->rtus_.reduce_src_) return;
    const auto &jcp = self->jcp_;
    const bool is_nspc
            = utils::one_of(jcp.src_tag, format_tag::nhwc, format_tag::nwc, format_tag::nchw, format_tag::ncw);
...
struct kdnn_rtus_driver_t : public kdnn_jit_generator {
    ...
    DECLARE_CPU_JIT_AUX_FUNCTIONS(kdnn_rtus_driver_t)
    ...
    kdnn_rtus_driver_t(int iw, int stride_w, int src_step_h, int src_step_icb,
        ...
        const int simd_w = vlen_ / sizeof(float16_t);
        ic_tail_ = ic_ % simd_w;
    }
    ...
    void loop_is_nspc() {
        ...
        const size_t load_store_size
                = isa == sve_256 ? vlen_ : max_load_store_bytes;
...
inline status_t kdnn_init_rtus_driver(conv_t *self) {
    ...
    const auto src_tag = memory_desc_wrapper(src_d).matches_one_of_tag(
            format_tag::nhwc, format_tag::nwc, format_tag::nchw, format_tag::ncw);
    ...
    CHECK(safe_ptr_assign(self->rtus_driver_,
            new kdnn_rtus_driver_t<isa>(iw, stride_w, src_step_h, src_step_icb,
                    ws_step_icb, src_to_ws, typesize, ic, is_nspc)));

    return self->rtus_driver_->create_kernel();
}

inline int kdnn_best_divider(int value, int min_divider, int max_divider,
...
typedef kdnn_jit_1x1_conv_conf_t kdnn_jcp_t;

inline bool kdnn_is_bcast_layout_nxc(const kdnn_jcp_t &jcp) {
...
inline bool kdnn_is_load_layout_nxc(const kdnn_jcp_t &jcp) {
    return jcp.prop_kind == prop_kind::backward_weights
            && utils::one_of(jcp.dst_tag, format_tag::ndhwc, format_tag::nhwc,
                    format_tag::nwc);
}

inline bool kdnn_is_out_layout_nxc(const kdnn_jcp_t &jcp) {
    switch (jcp.prop_kind) {
        case prop_kind::forward_training:
        case prop_kind::forward_inference:
            return utils::one_of(jcp.dst_tag, format_tag::ndhwc,
...
inline size_t kdnn_get_bcast_u_offset(const kdnn_jcp_t &jcp) {
    return kdnn_is_bcast_layout_nxc(jcp) ? jcp.ic : jcp.ic_block;
}

inline size_t kdnn_get_bcast_j_offset(const kdnn_jcp_t &jcp) {
    return kdnn_is_bcast_layout_nxc(jcp) ? jcp.reduce_dim : jcp.reduce_loop_unroll;
}

inline size_t kdnn_get_bcast_offset(const kdnn_jcp_t &jcp, int u, int j) {
    size_t offset;
    if (utils::one_of(jcp.prop_kind, prop_kind::forward_training,
                prop_kind::forward_inference, prop_kind::backward_data)) {
        assert(jcp.reduce_loop_unroll == jcp.reduce_block);
        if (kdnn_is_bcast_layout_nxc(jcp) || u != jcp.reduce_loop_unroll) {
            offset = j * kdnn_get_bcast_j_offset(jcp) + u;
        } else {
            offset = (jcp.bcast_dim + j) * kdnn_get_bcast_j_offset(jcp);
        }
    } else {
        offset = u * kdnn_get_bcast_u_offset(jcp) + j;
    }
    return sizeof(float16_t) * offset;
}

inline size_t kdnn_get_load_u_offset(const kdnn_jcp_t &jcp) {
    return kdnn_is_load_layout_nxc(jcp) ? jcp.oc : jcp.oc_block;
}

inline size_t kdnn_get_load_i_offset(const kdnn_jcp_t &jcp) {
    return kdnn_is_load_layout_nxc(jcp) ? jcp.oc_block : jcp.os;
}

inline size_t kdnn_get_load_bwd_w_offset(const kdnn_jcp_t &jcp, int i, int u0) {
    if (kdnn_is_load_layout_nxc(jcp)) {
        return i * kdnn_get_load_i_offset(jcp) + u0 * kdnn_get_load_u_offset(jcp);
    } else {
        return (i * kdnn_get_load_i_offset(jcp) + u0) * kdnn_get_load_u_offset(jcp);
    }
}

inline size_t kdnn_get_output_i_offset(const kdnn_jcp_t &jcp) {
    if (kdnn_is_out_layout_nxc(jcp)) {
...
inline size_t kdnn_get_output_j_offset(const kdnn_jcp_t &jcp) {
    return kdnn_is_out_layout_nxc(jcp) ? jcp.load_dim : jcp.load_block;
}

inline size_t kdnn_get_load_loop_output_fwd_offset(
        const kdnn_jcp_t &jcp, int load_loop_blk) {
    size_t offset = load_loop_blk * jcp.oc_block * sizeof(float16_t);
    if (!kdnn_is_out_layout_nxc(jcp)) {
...
inline size_t kdnn_get_load_loop_output_bwd_d_offset(
        const kdnn_jcp_t &jcp, int load_loop_blk) {
    size_t offset = load_loop_blk * jcp.ic_block * sizeof(float16_t);
    if (!kdnn_is_out_layout_nxc(jcp)) { offset *= jcp.os; }
    return offset;
}
