...
#include "cpu/aarch64/kdnn/jit/kdnn_jit_f16_sve_convolution.hpp"
...
void kdnn_jit_f16_sve_convolution_fwd_t<isa>::prepare_padded_bias(
        const float16_t *&bias,
        const memory_tracking::grantor_t &scratchpad) const {
    if (!pd()->wants_padded_bias()) return;

    auto padded_bias
            = scratchpad.template get<float16_t>(key_conv_padded_bias);
    utils::array_copy(padded_bias, bias, pd()->jcp_.oc_without_padding);
    utils::array_set(padded_bias + pd()->jcp_.oc_without_padding, (float16_t)0,
            pd()->jcp_.oc - pd()->jcp_.oc_without_padding);
    bias = padded_bias;
}

template <cpu_isa_t isa>
void kdnn_jit_f16_sve_convolution_fwd_t<isa>::execute_forward_1d(const exec_ctx_t &ctx) const {
    auto src = CTX_IN_MEM(const float16_t *, DNNL_ARG_SRC);
    auto weights = CTX_IN_MEM(const float16_t *, DNNL_ARG_WEIGHTS);
    auto bias = CTX_IN_MEM(const float16_t *, DNNL_ARG_BIAS);
...
template <cpu_isa_t isa>
void kdnn_jit_f16_sve_convolution_fwd_t<isa>::execute_forward_2d(const exec_ctx_t &ctx) const {
    auto src = CTX_IN_MEM(const float16_t *, DNNL_ARG_SRC);
    auto weights = CTX_IN_MEM(const float16_t *, DNNL_ARG_WEIGHTS);
    auto bias = CTX_IN_MEM(const float16_t *, DNNL_ARG_BIAS);
    auto dst = CTX_OUT_MEM(float16_t *, DNNL_ARG_DST);
...
template <cpu_isa_t isa>
void kdnn_jit_f16_sve_convolution_fwd_t<isa>::execute_forward_3d(const exec_ctx_t &ctx) const {
    auto src = CTX_IN_MEM(const float16_t *, DNNL_ARG_SRC);
    auto weights = CTX_IN_MEM(const float16_t *, DNNL_ARG_WEIGHTS);
    auto bias = CTX_IN_MEM(const float16_t *, DNNL_ARG_BIAS);
    auto dst = CTX_OUT_MEM(float16_t *, DNNL_ARG_DST);
...
template struct kdnn_jit_f16_sve_convolution_fwd_t<sve_256>;
